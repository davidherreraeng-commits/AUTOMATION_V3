...
class FakeProbe:
    ...
    def probe_general_validation(self, *, contract, **kwargs):
...
def test_should_map_general_validation_outcome() -> None:
    current_batch, item = batch()
    fake_probe = FakeProbe()

    outcome = service(current_batch, fake_probe).run_general_validation(
...
def test_should_reject_item_outside_batch() -> None:
    ...
    with pytest.raises(BatchPortalProbeBlockedError):
        service(current_batch, FakeProbe()).run_general_validation(
            batch_id=current_batch.batch_id,
            item_id=uuid4(),
            dependency="Adquisiciones",
        )
...
def test_should_preserve_ready_batch_after_general_validation() -> None:
    current_batch, item = batch()
