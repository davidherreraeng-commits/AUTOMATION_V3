...
def test_should_validate_selected_header_and_map_c3_outcome() -> None:
    ...
    assert outcome.contract_number == "80-2026"
    assert outcome.validate_clicked is True
    assert outcome.header_validation_confirmed is True
    assert outcome.general_data_ready is True
...
def test_should_reject_header_validation_for_item_outside_batch() -> None:
...
def test_should_preserve_ready_batch_after_header_validation_probe() -> None:
    current_batch, item = batch()
