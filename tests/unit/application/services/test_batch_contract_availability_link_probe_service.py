...
class FakeProbe:
    name = "fake-contract-availability-link"

    def __init__(self) -> None:
        self.contract = None

    def probe_contract_availability_link(self, *, contract, **kwargs):
        self.contract = contract
        return BatchContractAvailabilityLinkProbeResult(
            success=True,
            code="CONTRACT_AVAILABILITY_LINK_READY",
            message="CDP vinculado.",
            authenticated=True,
            ...
            supervisor_success_dialog_found=True,
            supervisor_success_dialog_accepted=True,
            supervisor_linked_confirmed=True,
            availability_section_found=True,
            availability_search_written=True,
            availability_result_found=True,
            availability_result_matches=True,
            availability_link_clicked=True,
            availability_link_success_found=True,
...
def test_should_save_link_supervisor_and_link_cdp() -> None:
    ...
    outcome = service(
        current_batch,
        fake_probe,
    ).run_contract_availability_link(
        batch_id=current_batch.batch_id,
        item_id=item.item_id,
        dependency="Adquisiciones",
        confirmation="GUARDAR SUPERVISOR Y CDP 86-2026",
        allow_test_values=True,
    )

    assert fake_probe.contract == item.contract
    assert outcome.code == "CONTRACT_AVAILABILITY_LINK_READY"
    assert outcome.cdp_code == "700"
    assert outcome.supervisor_linked_confirmed is True
    assert outcome.availability_linked_row_confirmed is True
    assert outcome.budget_register_section_found is True
    assert current_batch.status is BatchStatus.READY


def test_should_reject_incorrect_availability_confirmation() -> None:
    current_batch, item = batch()

    with pytest.raises(
        BatchPortalProbeBlockedError,
        match="GUARDAR SUPERVISOR Y CDP 86-2026",
    ):
        service(
            current_batch,
            FakeProbe(),
            ...
            item_id=item.item_id,
            dependency="Adquisiciones",
            confirmation="GUARDAR Y VINCULAR 86-2026",
            allow_test_values=True,
        )


def test_should_require_test_value_authorization_for_cdp_flow() -> None:
    current_batch, item = batch()

    with pytest.raises(
        BatchPortalProbeBlockedError,
        match="allow_test_values",
    ):
        service(
            current_batch,
            FakeProbe(),
