...
class FakeProbe:
    name = "fake-general-completion-draft"
    ...
    def probe_general_data_draft(self, **kwargs):
        return BatchGeneralDataDraftProbeResult(True, "READY", "Listo")

    def probe_general_completion_draft(self, *, contract, **kwargs):
        self.contract = contract
        return BatchGeneralCompletionDraftProbeResult(
            success=True,
            code="GENERAL_COMPLETION_DRAFT_READY",
            message="C4 completo sin validar ni guardar.",
            authenticated=True,
...
def test_should_map_general_completion_draft_outcome() -> None:
    current_batch, item = batch()
    fake_probe = FakeProbe()

    outcome = service(current_batch, fake_probe).run_general_completion_draft(
    ...
    assert outcome.general_completion_completed is True
    assert outcome.budget_year == 2026
    assert outcome.budget_item == "IDEA-2026"
    assert outcome.execution_department == "Antioquia"
    assert outcome.execution_city == "Medellín"
...
def test_should_reject_item_outside_batch() -> None:
    ...
    with pytest.raises(BatchPortalProbeBlockedError):
        service(current_batch, FakeProbe()).run_general_completion_draft(
            batch_id=current_batch.batch_id,
            item_id=uuid4(),
            dependency="Adquisiciones",
        )
...
def test_should_preserve_ready_batch_after_general_completion_probe() -> None:
    current_batch, item = batch()
