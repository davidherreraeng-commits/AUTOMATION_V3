...
class FakeContractPortal:
    ...
    def verify_step(
        ...
        errors = self.verification_errors.get(step)
        if errors:
            raise errors.pop(0)
...
def test_retry_should_reconcile_applied_step_before_reexecuting(
    tmp_path: Path,
) -> None:
    (
        executor,
    ...
    portal.verification_errors[
        ContractStep.ASSISTANT_OPENED
    ] = [PortalTimeoutError("La verificación no respondió.")]
    ...
    assert first.outcome is StepExecutionOutcome.RETRY_PENDING
    assert portal.execute_calls.count(ContractStep.ASSISTANT_OPENED) == 1
    assert ContractStep.ASSISTANT_OPENED in portal.applied_steps

    resumed = executor.execute_next(
        execution_id=execution.execution_id,
        contract=contract,
    )

    assert resumed.outcome is StepExecutionOutcome.STEP_RECONCILED
    assert resumed.step is ContractStep.ASSISTANT_OPENED
    assert portal.execute_calls.count(ContractStep.ASSISTANT_OPENED) == 1
    assert portal.verify_calls.count(ContractStep.ASSISTANT_OPENED) == 2

    stored = repository.get_by_id(execution.execution_id)
    assert stored is not None
    assert stored.status is ExecutionStatus.RUNNING
    assert stored.last_completed_step is ContractStep.ASSISTANT_OPENED


def test_should_reconcile_interrupted_input_validation_without_portal(
    tmp_path: Path,
) -> None:
    (
        executor,
    ...
    checkpoints.start_attempt(execution.execution_id)
    opened = checkpoints.begin_next_step(execution.execution_id)
    assert opened.current_step is ContractStep.INPUT_VALIDATED
    ...
    assert resumed.outcome is StepExecutionOutcome.STEP_RECONCILED
    assert resumed.step is ContractStep.INPUT_VALIDATED
    assert portal.execute_calls == []
    assert portal.verify_calls == []

    stored = repository.get_by_id(execution.execution_id)
    assert stored is not None
    assert stored.last_completed_step is ContractStep.INPUT_VALIDATED
    assert stored.current_step is None
