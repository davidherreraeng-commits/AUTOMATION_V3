    ...
    def probe_header_validation(
        ...
        *,
        ...
        return BatchHeaderValidationProbeResult(
            ...
            message="C3 disponible sin guardar.",
            authenticated=True,
            ...
            general_object_found=True,
            general_signing_date_found=True,
            general_starting_date_found=True,
            general_amount_found=True,
            general_contract_term_found=True,
            save_clicked=False,
        )
...
def test_superuser_should_validate_header_without_saving(tmp_path: Path) -> None:
    probe = FakeProbe()
    app = create_app(
        settings(tmp_path),
    ...
    with TestClient(app) as client:
        ...
        response = client.post(
            ...
            "/execution/header-validation-probe",
            json={"item_id": item_id},
        )

        assert response.status_code == 200
        payload = response.json()
        assert payload["code"] == "HEADER_VALIDATION_READY"
        assert payload["validate_clicked"] is True
        assert payload["header_validation_confirmed"] is True
        assert payload["general_data_ready"] is True
...
def test_should_reject_validation_for_contract_outside_batch(
    tmp_path: Path,
) -> None:
    app = create_app(
        settings(tmp_path),
...
def test_operator_should_not_validate_header(tmp_path: Path) -> None:
    app = create_app(
        settings(tmp_path),
        portal_credential_verifier=FakeVerifier(),
        batch_portal_probe=FakeProbe(),
