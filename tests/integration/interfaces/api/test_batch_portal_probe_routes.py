...
class FakePortalProbe:
    ...
    def probe(self, *, portal_username: str, portal_password: str):
        ...
        return BatchPortalProbeResult(
            ...
            message="Login y navegación confirmados sin abrir el formulario.",
            authenticated=True,
...
def test_superuser_should_run_read_only_portal_probe(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        response = client.post(
            f"/api/v1/batches/{batch['batch_id']}/execution/probe"
        )

        assert response.status_code == 200
        payload = response.json()
        assert payload["success"] is True
        assert payload["code"] == "NAVIGATION_READY"
        assert payload["assistant_access_found"] is True
        assert payload["probe_name"] == "fake-read-only-probe"
...
def test_operator_should_not_run_portal_probe(tmp_path: Path) -> None:
    app = create_app(
        settings(tmp_path),
        portal_credential_verifier=FakeVerifier(),
...
def test_cancelled_batch_should_reject_portal_probe(tmp_path: Path) -> None:
    app = create_app(
        settings(tmp_path),
        portal_credential_verifier=FakeVerifier(),
        batch_portal_probe=FakePortalProbe(),
