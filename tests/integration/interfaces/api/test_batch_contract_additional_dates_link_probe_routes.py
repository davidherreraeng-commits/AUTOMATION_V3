...
class FakeProbe:
    name = "fake-additional-dates"

    def probe_contract_additional_dates_link(self, **kwargs):
...
def test_superuser_should_link_additional_dates(tmp_path: Path):
    app = create_app(
        settings(tmp_path),
    ...
    with TestClient(app) as client:
        ...
        response = client.post(
            ...
            "/execution/contract-additional-dates-link-probe",
            json={
                "item_id": current_batch["contracts"][0]["item_id"],
                "confirmation": (
                    "GUARDAR SUPERVISOR CDP RP Y FECHAS 90-2026"
                ),
        ...
        assert payload["code"] == "CONTRACT_ADDITIONAL_DATES_LINK_READY"
        assert payload["guarantee_approval_date"] == "2026-08-03"
        assert payload["website_publication_date"] == "2026-08-03"
        assert payload["secop_publication_date"] == "2026-08-03"
        assert payload["file_reported_section_found"] is True
...
def test_operator_should_not_link_additional_dates(tmp_path: Path):
    app = create_app(
        settings(tmp_path),
        portal_credential_verifier=FakeVerifier(),
        batch_portal_probe=FakeProbe(),
