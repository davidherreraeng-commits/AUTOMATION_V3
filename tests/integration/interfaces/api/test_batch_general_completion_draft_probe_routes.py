...
def test_superuser_should_fill_c4_without_final_validation_or_save(
    tmp_path: Path,
) -> None:
    probe = FakeProbe()
    app = create_app(
    ...
    with TestClient(app) as client:
        ...
        response = client.post(
            ...
            "/execution/general-completion-draft-probe",
            json={"item_id": item_id},
        )

        assert response.status_code == 200
        payload = response.json()
        assert payload["code"] == "GENERAL_COMPLETION_DRAFT_READY"
        assert payload["general_data_completed"] is True
        assert payload["general_completion_completed"] is True
        assert payload["budget_link_clicked"] is True
        assert payload["secop_url_written"] is True
        assert payload["final_validate_button_found"] is True
...
def test_should_reject_c4_for_contract_outside_batch(tmp_path: Path) -> None:
    app = create_app(
        settings(tmp_path),
        portal_credential_verifier=FakeVerifier(),
...
def test_operator_should_not_fill_c4(tmp_path: Path) -> None:
    app = create_app(
        settings(tmp_path),
        portal_credential_verifier=FakeVerifier(),
        batch_portal_probe=FakeProbe(),
