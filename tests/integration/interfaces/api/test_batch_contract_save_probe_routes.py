...
def test_superuser_should_save_explicitly_authorized_test_contract(
    tmp_path: Path,
) -> None:
    app = create_app(
        settings(tmp_path),
    ...
    with TestClient(app) as client:
        ...
        assert payload["code"] == "CONTRACT_SAVE_READY"
        assert payload["amount"] == "1"
        assert payload["save_clicked"] is True
        assert payload["success_dialog_accepted"] is True
        assert payload["supervisor_section_found"] is True


def test_should_reject_incorrect_confirmation(tmp_path: Path) -> None:
    app = create_app(
        settings(tmp_path),
        portal_credential_verifier=FakeVerifier(),
        batch_portal_probe=FakeProbe(),
...
def test_operator_should_not_save_contract(tmp_path: Path) -> None:
    app = create_app(
        settings(tmp_path),
        portal_credential_verifier=FakeVerifier(),
        batch_portal_probe=FakeProbe(),
