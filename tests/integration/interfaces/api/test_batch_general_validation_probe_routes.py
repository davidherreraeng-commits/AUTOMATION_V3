    ...
    def probe_general_validation(
        ...
        *,
        ...
        return BatchGeneralValidationProbeResult(
            ...
            message="C3-C4 validados sin guardar.",
            authenticated=True,
...
def test_superuser_should_validate_general_form_without_saving(
    tmp_path: Path,
) -> None:
    probe = FakeProbe()
    app = create_app(
    ...
    with TestClient(app) as client:
        ...
        response = client.post(
            ...
            "/execution/general-validation-probe",
            json={"item_id": item_id},
        )

        assert response.status_code == 200
        payload = response.json()
        assert payload["code"] == "GENERAL_VALIDATION_READY"
        assert payload["general_completion_completed"] is True
        assert payload["general_validate_clicked"] is True
        assert payload["general_validation_confirmed"] is True
        assert payload["save_button_found"] is True
...
def test_should_reject_general_validation_for_contract_outside_batch(
    tmp_path: Path,
) -> None:
    app = create_app(
        settings(tmp_path),
...
def test_operator_should_not_validate_general_form(tmp_path: Path) -> None:
    app = create_app(
        settings(tmp_path),
        portal_credential_verifier=FakeVerifier(),
        batch_portal_probe=FakeProbe(),
