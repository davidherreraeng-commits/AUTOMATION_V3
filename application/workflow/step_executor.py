...
class StepExecutor:
    ...
    def execute_next(
        ...
        *,
        ...
        if execution.status is ExecutionStatus.RETRY_PENDING:
            retry_result = self._resume_retry_pending(
                execution=execution,
                contract=contract,
            )
        ...
        elif execution.status is ExecutionStatus.PENDING:
        ...
        if resume_state.requires_reconciliation:
            if resume_state.step is ContractStep.INPUT_VALIDATED:
                return self._confirm_input_validation(
    ...
    def _resume_retry_pending(
        self,
        *,
        execution: ContractExecution,
        contract: ContractData,
    ) -> StepExecutionResult | None:
        """Inicia un reintento reconciliando primero el paso fallido.

        ``mark_retry_pending`` conserva ``last_failed_step``. Antes de
        repetir una acción potencialmente irreversible, se abre de nuevo
        ese paso y se comprueba su postcondición en el portal.
        """

        failed_step = execution.last_failed_step

        restarted = self._checkpoints.start_attempt(
            execution.execution_id,
            portal_profile=self._portal_profile,
        )
        ...
        opened = self._checkpoints.begin_next_step(
            restarted.execution_id
        )

        if opened.current_step is not failed_step:
            return self._mark_retry_step_mismatch(
                execution=opened,
                expected_step=failed_step,
            )

        if failed_step is ContractStep.INPUT_VALIDATED:
            return self._confirm_input_validation(
                execution=opened,
                reconciled=True,
            )
    ...
    def _confirm_input_validation(
        self,
        *,
        execution: ContractExecution,
        reconciled: bool,
    ) -> StepExecutionResult:
        """Confirma INPUT_VALIDATED sin utilizar el portal."""

        confirmed_execution = self._checkpoints.confirm_current_step(
            execution.execution_id,
        ...
        return StepExecutionResult(
            ...
            outcome=(
                ...
                if reconciled
                else StepExecutionOutcome.STEP_CONFIRMED
            ),
            step=ContractStep.INPUT_VALIDATED,
            message=(
                "La validación de entrada pendiente fue reconciliada."
                if reconciled
                else "Los datos de entrada fueron validados."
            ),
        )

    def _mark_retry_step_mismatch(
        self,
        *,
        execution: ContractExecution,
        expected_step: ContractStep,
    ) -> StepExecutionResult:
        actual_step = execution.current_step
        error_info = ExecutionErrorInfo(
            code="RETRY_CHECKPOINT_STEP_MISMATCH",
            category=ErrorCategory.UNKNOWN,
            message=(
                "El paso abierto al iniciar el reintento no coincide con "
                "el último paso fallido. Se requiere revisión manual."
            ),
            retryable=False,
            metadata={
        ...
        manual_review = self._checkpoints.mark_manual_review(
            execution.execution_id,
            error_info,
        )
